...
public class AllBaby_F extends Fragment implements IBtnCallListener, onCheckedChanged, OnClickListener {
	...
	private Adapter_ListView_cart adapter;//适配器
	...
	private void initView(View view) {
		tv_goShop = (TextView) view.findViewById(R.id.tv_goShop);//去逛逛
		tv_cart_Allprice = (TextView) view.findViewById(R.id.tv_cart_Allprice);//合计
		tv_cart_buy_Ordel = (TextView) view.findViewById(R.id.tv_cart_buy_or_del);//结算
		tv_cart_buy_Ordel.setText(str_del);//没啥用
	...
	public void onAttach(Activity activity) {
		...
		super.onAttach(activity);//传递数据回调接口
	}
		...
		switch (view.getId()) {
			...
			case R.id.tv_cart_buy_or_del://点击结算/删除
				...
				if (tv_cart_buy_Ordel.getText().toString().equals("删除")) {
					...
					for(int i = 0; i<Data.arrayList_cart.size();i++){
						if (null!=listView_cart.getChildAt(i)&&((CheckBox) (listView_cart.getChildAt(i)).findViewById(R.id.cb_choice)).isChecked()) {
						    Data msgdate=new Data();
						   data = String.valueOf(Data.arrayList_cart.get(i).get("id"));
						   data2 = String.valueOf(Data.arrayList_cart.get(i).get("name"));
						   data3 = String.valueOf(Data.arrayList_cart.get(i).get("price"));
                    ...
                    Toast.makeText(getActivity(),data+"?"+data2+"?"+data3 , Toast.LENGTH_SHORT).show();
					adapter.notifyDataSetChanged();
