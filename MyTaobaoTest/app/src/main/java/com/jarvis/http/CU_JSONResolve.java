...
//					return hashMap;
	...
	//json是数组的时候,商品详情专用
	public static HashMap<String, Bean_ListView_ware> parseHashMap2_2(String jsonStr, String STR1_field[], String LIST1_field[], ArrayList<String[]> aL_STR2_field) {
		System.out.println("json解析");

		HashMap<String, Bean_ListView_ware> hashMap = new HashMap<String, Bean_ListView_ware>();
...
//					return hashMap;
			...
			if (STR1_field != null && STR1_field.length != 0)
				for (int j = 0; j < jsonArray.length(); j++) {
					JSONObject jO0 = jsonArray.getJSONObject(j);
					System.out.println("json解析1");
				//	for (int i = 0; i < STR1_field.length; i++) {
						int k = 0;
					Bean_ListView_ware bean = new Bean_ListView_ware(
							jO0.getString("id"),
							jO0.getString("name"),
							jO0.getString("price"),
							jO0.getString("sale_num"),
							jO0.getString("address"),
							jO0.getString("pic"));
	...
	//json是数组的时候,商品详情专用
	public static HashMap<String, Bean_ListView_ware_1> parseHashMap2_3(String jsonStr, String STR1_field[], String LIST1_field[], ArrayList<String[]> aL_STR2_field) {
		System.out.println("json解析");

		HashMap<String, Bean_ListView_ware_1> hashMap = new HashMap<String, Bean_ListView_ware_1>();
			...
			if (STR1_field != null && STR1_field.length != 0)
				for (int j = 0; j < jsonArray.length(); j++) {
					...
					Bean_ListView_ware bean = new Bean_ListView_ware(
							...
							jO0.getString("address"),
							jO0.getString("pic"));
					Bean_ListView_ware_1 bean1 = new Bean_ListView_ware_1(bean);
					hashMap.put(bean1.getTv_price(), bean1);
					//}
				}
